...
def test_json_syntax_error():
    ...
    with pytest.raises(quacklab.ParserException, match="SYNTAX_ERROR", check=_parse_json_func("Parser Error: ")):
...
def test_json_catalog_error():
    conn = quacklab.connect()
    conn.execute("SET errors_as_json='true'")
    with pytest.raises(quacklab.CatalogException, match="MISSING_ENTRY", check=_parse_json_func("Catalog Error: ")):
