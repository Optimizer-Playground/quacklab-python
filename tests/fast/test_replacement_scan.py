...
def fetch_arrow_record_batch(rel: quacklab.DuckDBPyRelation):
...
def create_relation(conn, query: str) -> quacklab.DuckDBPyRelation:
    df = pd.DataFrame({"a": [1, 2, 3]})
...
class TestReplacementScan:
    ...
    def test_scan_global(self, duckdb_cursor):
        ...
        with pytest.raises(quacklab.CatalogException, match="Table with name global_polars_df does not exist"):
    ...
    def test_scan_local(self, duckdb_cursor):
        ...
        def inner_func(duckdb_cursor) -> None:
            duckdb_cursor.execute("set python_enable_replacements=false")
            with pytest.raises(quacklab.CatalogException, match="Table with name df does not exist"):
    ...
    def test_replacement_disabled(self):
        ...
        with pytest.raises(
            quacklab.InvalidInputException, match="Cannot change enable_external_access setting while database is running"
        ):
            con.execute("set enable_external_access=true")

        # Create connection with external access disabled
        con = quacklab.connect(config={"enable_external_access": False})
        with pytest.raises(quacklab.CatalogException, match="Table with name df does not exist!"):
            create_relation(con, "select * from df")
    ...
    def test_replacement_of_cross_connection_relation(self):
        con1 = quacklab.connect(":memory:")
        con2 = quacklab.connect(":memory:")
