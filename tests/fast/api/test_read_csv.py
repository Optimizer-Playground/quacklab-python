...
import quacklab
from quacklab import CSVLineTerminator
...
class TestReadCSV:
    def test_using_connection_wrapper(self):
        rel = quacklab.read_csv(TestFile("category.csv"))
        res = rel.fetchone()
        print(res)
        assert res == (1, "Action", datetime.datetime(2006, 2, 15, 4, 46, 27))

    def test_using_connection_wrapper_with_keyword(self):
        rel = quacklab.read_csv(TestFile("category.csv"), dtype={"category_id": "string"})
        res = rel.fetchone()
        print(res)
        assert res == ("1", "Action", datetime.datetime(2006, 2, 15, 4, 46, 27))
    ...
    def test_delimiter_and_sep(self, duckdb_cursor):
        with pytest.raises(quacklab.InvalidInputException, match="read_csv takes either 'delimiter' or 'sep', not both"):
    ...
    def test_compression_wrong(self, duckdb_cursor):
        with pytest.raises(quacklab.Error, match="Input is not a GZIP stream"):
    ...
    def test_quote(self, duckdb_cursor):
        with pytest.raises(
            quacklab.Error, match='The methods read_csv and read_csv_auto do not have the "quote" argument'
        ):
    ...
    def test_encoding_wrong(self, duckdb_cursor):
        with pytest.raises(
            quacklab.BinderException, match="Copy is only supported for UTF-8 encoded files, ENCODING 'UTF-8'"
        ):
    ...
    def test_null_padding(self, duckdb_cursor):
        ...
        rel = quacklab.read_csv(TestFile("nullpadding.csv"), null_padding=False, header=False)
        res = rel.fetchall()
        assert res == [
            ("# this file has a bunch of gunk at the top",),
            ("one,two,three,four",),
        ...
        rel = quacklab.read_csv(TestFile("nullpadding.csv"), null_padding=True, header=False)
        res = rel.fetchall()
        assert res == [
            ("# this file has a bunch of gunk at the top", None, None, None),
            ("one", "two", "three", "four"),
    ...
    def test_read_csv_combined(self, duckdb_cursor):
        ...
        rel = quacklab.read_csv(CSV_FILE, skiprows=1, delimiter=",", quotechar='"', escapechar="\\", dtype=COLUMNS)
        res = rel.fetchall()
    ...
    def test_read_csv_names(self, tmp_path):
        ...
        with pytest.raises(quacklab.InvalidInputException, match="read_csv only accepts 'names' as a list of strings"):
            con.read_csv(file, names=True)

        with pytest.raises(quacklab.InvalidInputException, match="not possible to detect the CSV Header"):
            con.read_csv(file, names=["a", "b", "c", "d", "e"])

        # Duplicates are not okay
        with pytest.raises(quacklab.BinderException, match="names must have unique values"):
    ...
    def test_read_csv_names_mixed_with_dtypes(self, tmp_path):
        ...
        with pytest.raises(quacklab.BinderException, match=expected_error):
            rel = con.read_csv(
                file,
                names=["a", "b", "c"],
                dtype={
    ...
    def test_read_csv_empty_list(self):
        ...
        files = []
        with pytest.raises(
            quacklab.InvalidInputException, match="Please provide a non-empty list of paths or file-like objects"
        ):
            con.read_csv(files)
    ...
    def test_read_csv_list_invalid_path(self, tmp_path):
        ...
        with pytest.raises(quacklab.IOException, match='No files found that match the pattern "not_valid_path"'):
            con.read_csv(files)
    ...
    def test_read_csv_options(self, duckdb_cursor, options, tmp_path):
        ...
        if "hive_types" in options:
            with pytest.raises(quacklab.InvalidInputException, match=r"Unknown hive_type:"):
    ...
    def test_read_enum(self, tmp_path):
        ...
        with pytest.raises(quacklab.CatalogException, match="Type with name mood_2 does not exist!"):
    ...
    def test_strict_mode(self, tmp_path):
        ...
        with pytest.raises(quacklab.InvalidInputException, match="CSV Error on Line"):
            rel.fetchall()
    ...
    def test_thousands_separator(self, tmp_path):
        ...
        with pytest.raises(
            quacklab.BinderException, match="Unsupported parameter for THOUSANDS: should be max one character"
        ):
    ...
    def test_files_to_sniff_option(self, tmp_path):
        ...
        with pytest.raises(quacklab.ConversionException, match="Conversion Error"):
            rel.fetchall()
        rel = con.read_csv(file_path, files_to_sniff=-1)
