...
class TestToParquet:
    ...
    def test_compression_gzip(self, pd):
        ...
        csv_rel = quacklab.read_parquet(temp_file_name, compression="gzip")
        assert rel.execute().fetchall() == csv_rel.execute().fetchall()

    def test_field_ids_auto(self):
        temp_file_name = os.path.join(tempfile.mkdtemp(), next(tempfile._get_candidate_names()))  # noqa: PTH118
        rel = quacklab.sql("""SELECT {i: 128} AS my_struct""")
    ...
    def test_field_ids(self):
        temp_file_name = os.path.join(tempfile.mkdtemp(), next(tempfile._get_candidate_names()))  # noqa: PTH118
        rel = quacklab.sql("""SELECT 1 as i, {j: 128} AS my_struct""")
    ...
    def test_partition(self, pd, write_columns):
        ...
        result = quacklab.sql(f"FROM read_parquet('{temp_file_name}/*/*.parquet', hive_partitioning=TRUE)")
    ...
    def test_file_size_bytes_basic(self, file_size_bytes):
        ...
        rel = quacklab.from_query("SELECT i AS col_a, i AS col_b FROM range(0,10000) tbl(i);")
