...
def scoped_default(duckdb_cursor):
    default = quacklab.connect(":default:")
    quacklab.set_default_connection(duckdb_cursor)
    # Overwrite the default connection
    yield
    # Set it back on finalizing of the function
    quacklab.set_default_connection(default)
...
class TestRAPIQuery:
    ...
    def test_query_non_select_result(self, duckdb_cursor):
        with pytest.raises(quacklab.ParserException, match="syntax error"):
    ...
    def test_set_default_connection(self, scoped_default):
        quacklab.sql("create table t as select 42")
        assert quacklab.table("t").fetchall() == [(42,)]
        con = quacklab.connect(":default:")

        # Uses the same db as the module
        assert con.table("t").fetchall() == [(42,)]

        con2 = quacklab.connect()
        con2.sql("create table t as select 21")
        assert con2.table("t").fetchall() == [(21,)]
        # Change the db used by the module
        quacklab.set_default_connection(con2)

        with pytest.raises(quacklab.CatalogException, match="Table with name d does not exist"):
            con2.table("d").fetchall()

        assert quacklab.table("t").fetchall() == [(21,)]

        quacklab.sql("create table d as select [1,2,3]")

        assert quacklab.table("d").fetchall() == [([1, 2, 3],)]
    ...
    def test_set_default_connection_error(self, scoped_default):
        with pytest.raises(TypeError, match="Invoked with: None"):
            ...
            quacklab.set_default_connection(None)

        with pytest.raises(TypeError, match="Invoked with: 5"):
            quacklab.set_default_connection(5)

        assert quacklab.sql("select 42").fetchall() == [(42,)]
        quacklab.close()
        ...
        con3 = quacklab.connect()
        con3.close()
        quacklab.set_default_connection(con3)
