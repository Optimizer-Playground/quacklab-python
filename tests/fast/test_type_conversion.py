...
import quacklab
from quacklab.sqltypes import BIGINT, DOUBLE, FLOAT, HUGEINT, UHUGEINT, VARCHAR, DuckDBPyType
...
class TestIssue115FloatToUnion:
    ...
    def test_udf_float_to_union_type(self):
        ...
        conn.create_function(
            "return_float",
            lambda: 1.5,
            return_type=quacklab.union_type({"u1": VARCHAR, "u2": BIGINT, "u3": DOUBLE}),
        )
    ...
    def test_udf_int_to_ambiguous_union_type(self):
        ...
        conn.create_function(
            "return_int",
            lambda: 42,
            return_type=quacklab.union_type({"u1": VARCHAR, "u2": BIGINT, "u3": BIGINT}),
        )
    ...
    def test_udf_string_to_ambiguous_union_type(self):
        ...
        conn.create_function(
            "return_str",
            lambda: "hello",
            return_type=quacklab.union_type({"u1": VARCHAR, "u2": BIGINT, "u3": VARCHAR}),
        )
...
class TestIssue171DictKeyCaseSensitivity:
    ...
    def test_case_sensitive_dict_keys(self):
        result = quacklab.execute("SELECT ?", [{"Key": "first", "key": "second"}]).fetchone()[0]
        assert result["Key"] == "first"
        assert result["key"] == "second"

    def test_case_sensitive_dict_keys_three_variants(self):
        result = quacklab.execute("SELECT ?", [{"abc": 1, "ABC": 2, "Abc": 3}]).fetchone()[0]
        assert result["abc"] == 1
        assert result["ABC"] == 2
        assert result["Abc"] == 3
