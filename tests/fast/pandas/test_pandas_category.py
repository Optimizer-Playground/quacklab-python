...
def check_category_equal(category):
    ...
    df_out = quacklab.query_df(df_in, "data", "SELECT * FROM data").df()
...
class TestCategory:
    def test_category_simple(self, duckdb_cursor):
        ...
        df_out = quacklab.query_df(df_in, "data", "SELECT * FROM data").df()
        print(quacklab.query_df(df_in, "data", "SELECT * FROM data").fetchall())
        print(df_out["int"])
    ...
    def test_categorical_fetchall(self, duckdb_cursor):
        ...
        assert quacklab.query_df(df_in, "data", "SELECT * FROM data").fetchall() == [
            ("foo",),
            ("bla",),
            (None,),
            ("zoo",),
    ...
    def test_empty_categorical(self, duckdb_cursor):
        ...
        with pytest.raises(quacklab.ConversionException, match="Could not convert string 'test' to UINT8"):
