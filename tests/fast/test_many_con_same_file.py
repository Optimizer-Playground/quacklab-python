...
def test_multiple_writes_memory():
    con1 = quacklab.connect()
    con2 = quacklab.connect()
    con1.execute("CREATE TABLE foo1 as SELECT 1 as a, 2 as b")
    con2.execute("CREATE TABLE bar1 as SELECT 2 as a, 3 as b")
    con3 = quacklab.connect(":memory:")
...
def test_multiple_writes_named_memory():
    con1 = quacklab.connect(":memory:1")
    con2 = quacklab.connect(":memory:1")
    con1.execute("CREATE TABLE foo1 as SELECT 1 as a, 2 as b")
    con2.execute("CREATE TABLE bar1 as SELECT 2 as a, 3 as b")
    con3 = quacklab.connect(":memory:1")
    tbls = get_tables(con3)
    assert tbls == ["bar1", "foo1"]
    del con1
    del con2
...
def test_diff_config():
    con1 = quacklab.connect("test.db", False)
    with pytest.raises(
        quacklab.ConnectionException,
        match="Can't open a connection to same database file with a different configuration than existing connections",
    ):
        quacklab.connect("test.db", True)
    con1.close()
    del con1


def test_diff_config_extended():
    con1 = quacklab.connect("test.db", config={"null_order": "NULLS FIRST"})
    with pytest.raises(
        quacklab.ConnectionException,
        match="Can't open a connection to same database file with a different configuration than existing connections",
    ):
        quacklab.connect("test.db")
    con1.close()
    del con1
