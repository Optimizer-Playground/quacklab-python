...
class TestVariantFetchall:
    ...
    def test_integer(self):
        result = quacklab.sql("SELECT 42::VARIANT AS v").fetchone()
        assert result[0] == 42

    def test_string(self):
        result = quacklab.sql("SELECT 'hello'::VARIANT AS v").fetchone()
        assert result[0] == "hello"

    def test_boolean(self):
        result = quacklab.sql("SELECT true::VARIANT AS v").fetchone()
        assert result[0] is True

    def test_double(self):
        result = quacklab.sql("SELECT 3.14::DOUBLE::VARIANT AS v").fetchone()
        assert abs(result[0] - 3.14) < 1e-10

    def test_null(self):
        result = quacklab.sql("SELECT NULL::VARIANT AS v").fetchone()
        assert result[0] is None

    def test_list(self):
        result = quacklab.sql("SELECT [1, 2, 3]::VARIANT AS v").fetchone()
        assert result[0] == [1, 2, 3]

    def test_struct(self):
        result = quacklab.sql("SELECT {'a': 1, 'b': 2}::VARIANT AS v").fetchone()
        assert result[0] == {"a": 1, "b": 2}

    def test_nested_struct(self):
        result = quacklab.sql("SELECT {'x': {'y': 42}}::VARIANT AS v").fetchone()
        assert result[0] == {"x": {"y": 42}}

    def test_map(self):
        result = quacklab.sql("SELECT MAP {'key1': 'val1', 'key2': 'val2'}::VARIANT AS v").fetchone()
        val = result[0]
...
class TestVariantFetchNumpy:
    ...
    def test_single_row(self):
        result = quacklab.sql("SELECT 42::VARIANT AS v").fetchnumpy()
...
class TestVariantFetchDF:
    ...
    def test_basic(self):
        df = quacklab.sql("SELECT 42::VARIANT AS v").df()
...
class TestVariantArrow:
    ...
    def test_to_arrow_table(self):
        quacklab.sql("SELECT 42::VARIANT AS v").arrow()

    @pytest.mark.xfail(strict=True, reason="Arrow export for VARIANT not yet supported in DuckDB core")
    def test_fetch_arrow_reader(self):
        quacklab.sql("SELECT 42::VARIANT AS v").fetch_arrow_reader()

    @pytest.mark.xfail(strict=True, reason="Polars uses Arrow, which doesn't support VARIANT yet")
    def test_polars(self):
        quacklab.sql("SELECT 42::VARIANT AS v").pl()
...
class TestVariantType:
    ...
    def test_type_from_string(self):
        t = quacklab.type("VARIANT")
        assert t.id == "variant"

    def test_variant_constant(self):
        from quacklab.sqltypes import VARIANT
    ...
    def test_children_raises(self):
        t = quacklab.type("VARIANT")
        with pytest.raises(quacklab.InvalidInputException, match="not nested"):
            _ = t.children
...
class TestVariantPySpark:
    ...
    def test_variant_converts_to_variant_type(self):
        from quacklab.experimental.spark.sql.type_utils import convert_type
        from quacklab.experimental.spark.sql.types import VariantType
