...
class TestType:
    def test_sqltype(self):
        assert str(quacklab.sqltype("struct(a VARCHAR, b BIGINT)")) == "STRUCT(a VARCHAR, b BIGINT)"
    ...
    def test_list_type(self):
        type = quacklab.list_type(BIGINT)
        assert str(type) == "BIGINT[]"

    def test_array_type(self):
        type = quacklab.array_type(BIGINT, 3)
        assert str(type) == "BIGINT[3]"

    def test_struct_type(self):
        type = quacklab.struct_type({"a": BIGINT, "b": BOOLEAN})
    ...
    def test_implicit_convert_from_builtin_type(self):
        ...
        for duckdb_type, expected in mapping.items():
            res = quacklab.list_type(duckdb_type)
            assert str(res.child) == expected

        res = quacklab.list_type({"a": str, "b": int})
        assert str(res.child) == "STRUCT(a VARCHAR, b BIGINT)"

        res = quacklab.list_type(dict[str, int])
        assert str(res.child) == "MAP(VARCHAR, BIGINT)"

        res = quacklab.list_type(list[str])
        assert str(res.child) == "VARCHAR[]"

        res = quacklab.list_type(list[dict[str, dict[list[str], str]]])
        assert str(res.child) == "MAP(VARCHAR, MAP(VARCHAR[], VARCHAR))[]"

        res = quacklab.list_type(list[Union[str, int]])
    ...
    def test_attribute_accessor(self):
        type = quacklab.row_type([BIGINT, quacklab.list_type(quacklab.map_type(BLOB, BIT))])
    ...
    def test_json_type(self):
        json_type = quacklab.type("JSON")

        val = quacklab.Value('{"duck": 42}', json_type)
        res = quacklab.execute("select typeof($1)", [val]).fetchone()
        assert res == ("JSON",)

    def test_struct_from_dict(self):
        res = quacklab.list_type({"a": VARCHAR, "b": VARCHAR})
        assert res == "STRUCT(a VARCHAR, b VARCHAR)[]"

    def test_hash_method(self):
        type1 = quacklab.list_type({"a": VARCHAR, "b": VARCHAR})
        type2 = quacklab.list_type({"b": VARCHAR, "a": VARCHAR})
        type3 = VARCHAR
