...
class TestArrowUnregister:
    def test_arrow_unregister1(self, duckdb_cursor):
        ...
        with pytest.raises(quacklab.CatalogException, match="Table with name arrow_table does not exist"):
            connection.execute("SELECT * FROM arrow_table;").to_arrow_table()
        with pytest.raises(quacklab.CatalogException, match="View with name arrow_table does not exist"):
