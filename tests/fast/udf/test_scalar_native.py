...
class TestNativeUDF:
    def test_default_conn(self):
        ...
        quacklab.create_function("default_conn_passthrough", passthrough, [BIGINT], BIGINT)
        res = quacklab.sql("select default_conn_passthrough(5)").fetchall()
        assert res == [(5,)]
    ...
    def test_return_incorrectly_typed_object(self):
        ...
        with pytest.raises(
            quacklab.InvalidInputException, match="Failed to cast value: Could not convert string 'duckdb' to INT64"
        ):
    ...
    def test_structs(self):
        ...
        con.create_function(
            "append_field",
            add_extra_column,
            [quacklab.struct_type({"a": BIGINT, "b": BIGINT})],
            quacklab.struct_type({"a": BIGINT, "b": BIGINT, "c": BIGINT}),
        )
