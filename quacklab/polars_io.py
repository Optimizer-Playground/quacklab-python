...
def _predicate_to_expression(predicate: pl.Expr) -> quacklab.Expression | None:
...
def _tree_to_sql_expression(tree: _ExpressionTree) -> quacklab.Expression | None:
    ...
    """
    try:
        return quacklab.SQLExpression(_pl_tree_to_sql(tree))
    except Exception:
...
def _pl_tree_to_sql(tree: _ExpressionTree) -> str:
    ...
    if node_type == "Scalar":
        ...
        if dtype == "String" or dtype == "StringOwned":
            ...
            return str(quacklab.ConstantExpression(string_val))
...
def duckdb_source(relation: quacklab.DuckDBPyRelation, schema: pl.schema.Schema) -> pl.LazyFrame:
